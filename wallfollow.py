...
class LaserTurn:
    def __init__(self, state):
        self.state = state
        self.targetangle = self.get_target_angle()  # edit
        self.done = False
    ...
    def get_target_angle(self):
        ...
        targetangle = rectify_angle_pi(targetangle)
...
class WallFollow:
    ...
    def correct(self):
        ...
        closest, index = correct_turn.get_closest_info()

        # if closest > 1.05:
        #     start_angle = correct_turn.targetangle
        #
        # if closest < .95:
        #     start_angle = correct_turn.targetangle - pi

        # turn = Turn(self.state, start_angle)
        # while not turn.done:
        #     turn.act()
        #     self.rate.sleep()
        #
        # dist = np.abs(closest - 1)
        # print(dist)
        # forward = Forward(self.state, dist)
        # while not forward.done:
        #     forward.act()
        #     self.rate.sleep()
        # print("SUCCESSFULLY ENDED")
        turn_angle = correct_turn.targetangle - pi/2
        turn = Turn(self.state, turn_angle)
        while not turn.done:
            turn.act()
            self.rate.sleep()
        print("SUCCESSFULLY ENDED AGAIN")
        self.state.cmd_vel.publish(Twist())
    ...
    def act(self):
        ...
        move_cmd.linear.x = .1  # error? meters - driven so far #when to stop?

        error = closest - 1
        target_angle = laser_turn.targetangle - pi/2

        if error > 0.1:
            target_angle += error *4
            # turn right more
        ...
        provisional = target_angle - self.state.angle
        if provisional != 0:
            if provisional >= pi:
                self.turn = provisional - 2 * np.pi
            elif provisional <= -pi:
            ...
            self.targetangle = rectify_angle_pi(target_angle)
            ...
            if (error > .02):
                print("I AM CORRECTING MY ANGLE")
                if self.direction == 'L':
                    move_cmd.angular.z = compute_vel_from_angle(error)
